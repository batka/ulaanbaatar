...
<form action="<?=$REQUEST_URI?>" name="<?=$pageFormName;?>" id="<?=$pageFormName;?>" method="post">
	...
	<?php
		...
		for($i=0; $i<$rowcount; $i++){
			...
			$rowcount1 = $con->GetDescr("select count(*) from tbl_creationimage where CreationID='".$row[$i]['CreationID']."'");
	?>
		<div class="dcontent">
			<div class="descr">
			    <?php
			    ...
			    ?>
			    	<a href="<?=$link?>" class="descrtitle"><img class="dcontentimg" src="<?=$rfo?>/files/creation/medium/<?=$row[$i]['ImageSource']?>" width="200"/></a>
			    <?php }else{?>
