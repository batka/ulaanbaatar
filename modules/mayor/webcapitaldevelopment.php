...
<?php
	...
	$page_link="$rf/capital/development/page/";
	$showcount=10;
			...
			<td width="600" valign="top">
				<div class="formcenter">
					...
					<?php
						...
						$allrow=$con->GetDescr("select count(*) from mayor_capitaldevelopment where IsShow='YES'");
						$qry.=" limit ".($pagenum*$showcount).", ".$showcount;
						$row=$con->select($qry);
						$rowcount=count($row);
						if($rowcount>0){
						for($i=0; $i<$rowcount; $i++){
							$link="$rf/capital/development/detail/".$row[$i]['CapitalDevelopmentID']."/";
					?>
						<div class="dcontent">
							...
							<div class="descr">
							<?php if(!empty($row[$i]['ImageSource']) && file_exists("$drf/images/capital/capdevelopment/".$row[$i]['ImageSource'])){?>
								<a href="<?=$link?>"><img class="dcontentimg" src="<?=$rf?>/images/capital/capdevelopment/small/<?=$row[$i]['ImageSource']?>" height="120"/></a>
							<?php }
