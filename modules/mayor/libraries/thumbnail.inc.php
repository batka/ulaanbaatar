	...
	public function cropdim($width, $height) {
		...
		*/
		...
		if ($width > $height) {
			...
			imagecopyresampled ( $this->workingImage, $this->oldImage, - $int_width, 0, 0, 0, $adjusted_width, $height, $this->currentDimensions ['width'], $this->currentDimensions ['height'] );
		} elseif (($width < $height) || ($width == $height)) {
			$adjusted_height = $height / $wm;
			$half_height = $adjusted_height / 2;
			$int_height = $half_height - $h_height;
			imagecopyresampled ( $this->workingImage, $this->oldImage, 0, - $int_height, 0, 0, $width, $adjusted_height, $this->currentDimensions ['width'], $this->currentDimensions ['height'] );
		} else {
			imagecopyresampled ( $this->workingImage, $this->oldImage, 0, 0, 0, 0, $width, $height, $this->currentDimensions ['width'], $$this->currentDimensions ['height'] );
		}
