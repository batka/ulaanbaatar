<?php
	$page_link="$rf/council/nzdtg/page/";
	$showcount=10;
...
<div class="formcenter">
	...
	<?php
		...
		$qry.=" and PositionID='108'";
		$qry.=" order by CreateDate desc";
		$allrow=$con->GetDescr("select count(*) from mayor_councilconsist where IsShow='YES' and PositionID='106'");
		$qry.=" limit ".($pagenum*$showcount).", ".$showcount;
		$row=$con->select($qry);
		$rowcount=count($row);
		if($rowcount>0){
			if($rowcount==1){
				GotoPage("","$rf/council/nzdtg/detail/".$row[0]['CouncilManID']."/");
			}
		for($i=0; $i<$rowcount; $i++){
			$link="$rf/council/nzdtg/detail/".$row[$i]['CouncilManID']."/";
	?>
		<div class="dcontent">
			<div class="descr">
