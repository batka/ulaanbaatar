...
function OpenWindow1(url,p_height,p_width,p_top){

	if (!querywindow.closed && querywindow.location) {
	querywindow.focus();
	} else {
	//alert(url+'&scrolltop='+p_top);
	querywindow=window.open(url,'','addressbar=0,toolbar=0,location=0,directories=0,status=1,menubar=0,scrollbars=yes,resizable=no,top=1,left=1,width='+p_width+',height='+p_height);
	}
