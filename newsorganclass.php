<?php
	$contentname=substr($pagelink1, strlen($rf."/"));
	$show=$con->GetDescr("select count(*) from asu_leftmenu where IsShow='YES' and LeftMenuContent='$contentname'");
	if($show>0){
?>
<div id="column_2">
	<div id="menu_wrapper">
...
?>
					...
					<div class="level1selected"><a href="<?=$pagelink1;?>/<?=$row[$j]['OrganClassID'];?>"><span><?=$row[$j]['OrganClassName'];?></span></a></div>
...
?>
								<li class="pcml2selected rdcmsSHOW">
									<div class="overFOR"><a <?php if($organid==$row1[$j1]['OrganID'])echo ' class="active" ';?> href="<?=$pagelink1;?>/<?=$row[$j]['OrganClassID'];?>/<?=$row1[$j1]['OrganID'];?>" title=""><span><?=$row1[$j1]['OrganName'];?></span></a></div>
								</li>
<?php
	} 
?>
